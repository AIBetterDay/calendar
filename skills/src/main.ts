...
type Status = RawRow['status'];
type Priority = RawRow['priority'];
...
function format(row: RawRow): TodoOut {
  let tags: string[] = [];
  try { tags = JSON.parse(row.tags || '[]'); } catch { tags = []; }
  let repeatRule: unknown = { type: 'none' };
  try { repeatRule = JSON.parse(row.repeat_rule || '{"type":"none"}'); } catch { /* keep default */ }
  return {
    id: row.id,
    title: row.title,
...
async function findById(id: string): Promise<RawRow | null> {
...
const hasTime = (v: string | null | undefined): v is string => !!v && v.includes('T');
const addMinutes = (iso: string, minutes: number): string | null => {
...
function derivePlacement(input: {
  ...
  dueDate?: string | null;
  remindAt?: string | null;
}): { dateStart: string | null; dueDate: string | null } {
...
conn.registerSkill<ListParams, { items: TodoOut[]; meta: { total: number; page: number; pageSize: number } }>({
  ...
  handler: async (params, _ctx) => {
    ...
    if (params.priority) { wheres.push('priority = ?'); args.push(params.priority); }
    ...
    if (params.tag) { wheres.push('tags LIKE ?'); args.push(`%"${params.tag}"%`); }
...
conn.registerSkill<CreateParams, TodoOut & { _undo: { id: string } }>({
  ...
  reverse: async (input: unknown) => {
    const id = (input as { _undo?: { id?: string }; id?: string })?._undo?.id
...
interface UpdateResult extends TodoOut {
  _undo: { snapshot: RawRow };
}
...
conn.registerSkill<UpdateParams, UpdateResult>({
  ...
  handler: async (input) => {
    ...
    if (input.dateStart !== undefined) push('date_start', input.dateStart);
    if (input.remindAt !== undefined) push('remind_at', input.remindAt);
    if (input.reminderEnabled !== undefined) push('reminder_enabled', input.reminderEnabled ? 1 : 0);
    if (input.repeatRule !== undefined) push('repeat_rule', JSON.stringify(input.repeatRule));
    if (input.parentId !== undefined) push('parent_id', input.parentId);
  ...
  reverse: async (input: unknown) => {
    const snap = (input as { _undo?: { snapshot?: RawRow }; snapshot?: RawRow })?._undo?.snapshot
      ?? (input as { snapshot?: RawRow })?.snapshot;
    if (!snap?.id) return;
...
conn.registerSkill<BulkUpdateParams, BulkUpdateResult>({
    ...
    for (const id of ids) {
      ...
      if (params.status !== undefined) {
        ...
        if (params.status === 'completed') {
          push('completed_at', now); push('reminder_enabled', 0); push('remind_at', null);
        } else {
          push('completed_at', null);
        }
      }
      ...
      if (due !== undefined) push('due_date', due);
      const ds = normalizeNullable(params.dateStart ?? undefined);
      if (ds !== undefined) push('date_start', ds);
      const ra = normalizeNullable(params.remindAt ?? undefined);
      if (ra !== undefined) push('remind_at', ra);
      if (params.reminderEnabled !== undefined) push('reminder_enabled', params.reminderEnabled ? 1 : 0);
      if (params.color !== undefined) push('color', params.color);

      if (addTags.length || removeTags.length) {
        let tags: string[] = [];
        try { tags = JSON.parse(before.tags || '[]'); } catch { tags = []; }
  ...
  reverse: async (input: unknown) => {
    const snaps = (input as { _undo?: { snapshots?: RawRow[] }; snapshots?: RawRow[] })?._undo?.snapshots
      ?? (input as { snapshots?: RawRow[] })?.snapshots
      ?? [];
...
async function restoreRow(row: RawRow): Promise<void> {
